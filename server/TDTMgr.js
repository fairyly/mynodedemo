...
tdt.FindCouponsByFilter = function (filter, cb) {
    warehouse.FindCouponsByFilter_wh(filter, cb);
}
tdt.getSortFun = function (order, sortBy) {
    warehouse.getSortFun(order, sortBy);
}


tdt.urlToTpwd = function (url, text, logo, cb) {
    warehouse.urlToTpwd_wh(url, text, logo, cb);
}

tdt.getRecommend = function (num_iid, count, cb) {
    warehouse.getRecommend_wh(num_iid, count, cb);
}
